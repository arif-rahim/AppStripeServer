...
app.post("/buy", async (req, res) => {
  try {
    ...
    let { customer, receipt_email, amount, currency } = req.body;
    // Simple validation
    if (!customer || !receipt_email || !amount)
      return res.status(400).json({ message: "Invalid data" });
    amount = parseInt(amount);

    // Initiate payment customer:customer,
    const paymentIntent = await stripe.paymentIntents.create({
      ...
      metadata: { customer, receipt_email, amount },
    });
